...
// Helper to compress image if above target size (e.g., 5MB)
const compressIfNeeded = async (inputPath, outputPath, maxSizeMB = 5) => {
  ...
  await sharp(inputPath)
    .resize({ width: 2000, withoutEnlargement: true }) // optional max dimension
    .jpeg({ quality: 80 })
    .toFile(outputPath);
};
...
let posters = await Poster.find({ isTrending: { $ne: true } }).sort({ createdAt: -1 });
    let translationMap = {};
...
const compressImage = async (inputPath, outputPath) => {
  try {
    ...
    // always delete the raw temp file, even if compression fails
    fs.unlink(inputPath, () => {});
  }
...
export const canvasCreatePoster = async (req, res) => {
  try {
    ...
    const uniqueName = (prefix) =>
      `${prefix}_${Date.now()}-${Math.round(Math.random() * 1e9)}.jpg`;
    ...
    [uploadDir, bgDir, overlayDir].forEach((dir) => {
    ...
    // ── main poster image ────────────────────────────────────────────────────
    const posterFile    = req.files.posterImage;
    const rawPosterPath = path.join(uploadDir, `raw_${Date.now()}${path.extname(posterFile.name)}`);
    const posterName    = uniqueName("poster");
    const posterPath    = path.join(uploadDir, posterName);

    await posterFile.mv(rawPosterPath);
    await compressImage(rawPosterPath, posterPath);
    ...
    if (req.files.bgImage) {
      const bgFile    = Array.isArray(req.files.bgImage) ? req.files.bgImage[0] : req.files.bgImage;
      const rawBgPath = path.join(bgDir, `raw_${Date.now()}${path.extname(bgFile.name)}`);
      const bgName    = uniqueName("bg");
      const bgPath    = path.join(bgDir, bgName);

      await bgFile.mv(rawBgPath);
      await compressImage(rawBgPath, bgPath);
    ...
    if (req.files.overlayImages) {
      ...
      for (const file of overlayFiles) {
        const rawPath     = path.join(overlayDir, `raw_${Date.now()}${path.extname(file.name)}`);
        const overlayName = uniqueName("overlay");
        const overlayPath = path.join(overlayDir, overlayName);

        await file.mv(rawPath);
        await compressImage(rawPath, overlayPath);
...
export const getWeeklyPosters = async (req, res) => {
  try {
    ...
    const [user, posters] = await Promise.all([
      ...
      Poster.find({
  categoryName: new RegExp(`^${todayLower}$`, "i"),
  isTrending: { $ne: true }
})
